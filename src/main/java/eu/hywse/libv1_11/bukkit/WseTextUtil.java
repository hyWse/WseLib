package eu.hywse.libv1_11.bukkit;

//import org.bukkit.WseChatColor;

import eu.hywse.libv1_11.bungee.WseChatColor;
...
public class WseTextUtil {
    ...
    */
    ...
    private static String getProgressBar(double percent, String format, String cfull, String ctriple, String chalf, String cempty, DecimalFormat decimalFormat) {
        ...
        int z1 = 0, z2 = 0;
        if (varStr.length() > 0) z1 = Integer.parseInt(varStr.substring(0, 1));
        if (varStr.length() > 1) z2 = Integer.parseInt(varStr.substring(1, 2));
    ...
    public static String repeatColor(String input, char colorSymbol) {
        ...
        for (int i = 0; i < splitted.length; i++) {
            ...
            // Append last color
            if (lastColor != ' ' && append) {
                result.append(String.valueOf(colorSymbol)).append(String.valueOf(lastColor));
            }
        }
    ...
    public static String toCamelCase(String input) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (c == '_' || c == ' ') {
                res += " ";
                if (chars.length >= (i + 1)) {
                    res += Character.toUpperCase(chars[i + 1]);
                    i++;
                }
                continue;
            }
