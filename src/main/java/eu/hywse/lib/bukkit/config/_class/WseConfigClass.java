package eu.hywse.lib.bukkit.config._class;

import eu.hywse.lib.bukkit.WseLocation;
import eu.hywse.lib.bukkit.WseSerialize;
  ...
  public void save() {
    ...
    for (Field field : getClass().getDeclaredFields()) {
      ...
      try {
        ...
        // Bukkit location
        if (obj != null && (field.getType() == Location.class
            || field.getType() == WseLocation.class)) {
          obj = new WseLocation((Location) obj).toString();
        }

        System.out.println(
            "[i] Saving \"" + field.getName() + "\" with val: \"" + (obj != null ? obj.toString()
                : "NULL") + "\"");
