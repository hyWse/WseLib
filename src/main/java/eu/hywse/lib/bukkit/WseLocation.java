...
import eu.hywse.lib.bukkit.config.WseConfig;
import eu.hywse.lib.bukkit.extension.IterateBlockConsumer;
...
public class WseLocation extends Location {
  ...
  public WseLocation(String string, String splitter) {
    super(Bukkit.getServer().getWorld(string.split(splitter)[0]),
        Double.parseDouble(string.split(splitter)[1]),
        Double.parseDouble(string.split(splitter)[2]),
        Double.parseDouble(string.split(splitter)[3]),
   ...
   */
  public static void iterateBlocks(Location startPoint, Location endPoint, int maxIterations,
      IterateBlockConsumer action) {

    if (startPoint == null || startPoint.getWorld() == null
        || endPoint == null || endPoint.getWorld() == null) {
      return;
    }
  ...
  /**
   * Iterates through every block in selection Default maxIterations = 1000
   *
   * @param startPoint First Block
   * @param endPoint   Second Block
   * @param action     Action for every block
   */
  public static void iterateBlocks(Location startPoint, Location endPoint,
      IterateBlockConsumer action) {
  ...
  /**
   * Returns location as string Default splitter = "//"
   *
  ...
  public String toString(String splitter) {
    return getWorld().getName() + splitter + getX() + splitter + getY() + splitter + getZ()
        + splitter + getYaw() + splitter + getPitch();
  }
  ...
  public WseLocation getSafeLocation() {
    ...
    if ((buttom.getType() != Material.AIR || top.getType() != Material.AIR) && top.getY() <= 256) {
      for (int i = buttom.getY(); i <= 256; i++) {
        Block bB = add(0, 1, 0).getBlock();
        Block bT = clone().add(0, 1, 0).getBlock();

        if (bB.getType() == Material.AIR && bT.getType() == Material.AIR) {
          break;
        }
      }
    }
