...
public abstract class WseCommand {
   ...
   * @throws WseCommandException In some checks, e.g. if the executor is a player, throw an
   *                             exception
   */
  public abstract void onPlayerCommand(Player player, Command command, String[] args)
      throws WseCommandException;
   ...
   *                             exception
   */
  public void onConsoleCommand(CommandSender sender, Command command, String[] args)
      throws WseCommandException {
  ...
  public void checkPermission(CommandSender sender, String permission) throws WseCommandException {
    ...
    throw new WseCommandException(
        "The player \"" + plugin.getName() + "\" lacks the following permissions: " + permission,
        WseCommandException.Reason.NO_PERMISSION, permission);
  }
  ...
  public void checkArgs(String[] args, String pattern) throws WseCommandException {
    ...
    for (String arg : args) {
      if (lastArgument.getType() == Argument.ArgType.TXT) {
        continue;
      }
      ...
      switch (lastArgument.getType()) {
        ...
        case INT:
          if (!WseStringUtil.isInteger(arg)) {
            throw new WseCommandException("Arg \"" + arg + "\" is not a valid integer!",
                WseCommandException.Reason.INVALID_ARGS, arg);
          }

        case DBL:
          if (!WseStringUtil.isDouble(arg)) {
            throw new WseCommandException("Arg \"" + arg + "\" is not a valid double!",
                WseCommandException.Reason.INVALID_ARGS, arg);
          }

        case BOOL:
          if (!WseStringUtil.isBoolean(arg)) {
            throw new WseCommandException("Arg \"" + arg + "\" is not a valid boolean!",
                WseCommandException.Reason.INVALID_ARGS, arg);
          }
      }
    }
  ...
  public void checkPlayer(CommandSender sender) throws WseCommandException {
    ...
    throw new WseCommandException("Sender is not a player!", WseCommandException.Reason.NOT_PLAYER,
        sender.getName());
  }
  ...
  private void handleException(CommandSender sender, String label, WseCommandException ex) {
    switch (ex.getReason()) {
      ...
      case INVALID_ARGS:
        sendMessage(sender,
            "Ungültiger Syntax!" + (ex.getExtraStr() != null ? " &8[&a/" + label + " " + ex
                .getExtraStr() + "&8]" : ""));
        break;
    ...
    public enum ArgType {
      ...
      public static ArgType getType(String s) {
        for (ArgType type : ArgType.values()) {
          if (type.name().toLowerCase().equalsIgnoreCase(s.toLowerCase())) {
            return type;
          }
        }
        return null;
