...
public class WseMySql extends WseMySqlWrapper {
  ...
  public WseMySql(@NonNull Plugin plugin, @NonNull String sqlInfoFile)
      throws WseSqlInfoFileException {
    super(plugin.getDescription().getName(), sqlInfoFile);
    this.plugin = plugin;
  }

  public WseMySql(@NonNull Plugin plugin, @NonNull String host, @NonNull String database,
      @NonNull String user, @NonNull String password, int port) {
    super(plugin.getDescription().getName(), host, database, user, password, port);
    this.plugin = plugin;
  }

  public WseMySql(@NonNull Plugin plugin, @NonNull String sqlInfoFile, @NonNull String database)
      throws WseSqlInfoFileException {
