 ...
 * <p>
 * Initialized with a constant bound the {@code DiscardingCollection} removes values when it reaches
 * it. A Deque is used to just remove the first/oldest elements. The decorator can be wrapped around
 * an existing Deque and may be fully used as a collection. But casting it to a Deque will result in
 * a {@code ClassCastException}.
 * <p>
 * The delegated object (which is a {@code LinkedList} by default) can not  be accessed from outside
 * of the package and it should not be used since that could cause the class to be useless. Note
 * that {@code DiscardingCollections} are no codeDeques! They just use those to be able to remove
 * the oldest element.
 *
...
public class DiscardingCollection<E> extends ForwardingCollection<E> implements Collection<E> {
  ...
  /**
   * The maximum amount of elements that can be removed from a collection at once. This is needed
   * when a lot of elements are added in one operation. It should prevent not ending loops.
   */
  private static final int MAX_MASS_REMOVAL = 200;

  /**
   * The collections upper size bound. If its size is getting bigger than this value when modifying
   * the collection the oldest values are discarded.
   */
