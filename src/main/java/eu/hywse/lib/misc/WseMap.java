...
public class WseMap<K, V> extends HashMap<K, V> {
  ...
  public void printf(int maxKey, int maxVal) {
    ...
    for (Entry<K, V> entry : super.entrySet()) {
      ...
      if (key.length() > longestKey) {
        key = key.substring(0, longestKey - 1) + "$";
      }

      /* Value */
      String val = entry.getValue().toString();
      if (val.length() > longestVal) {
        val = val.substring(0, longestVal - 1) + "$";
      }
  ...
  /**
   * end the keys and values from the map in a fancy format = Unlimited length
   */
  public void printf() {
    printf(-1, -1);
  }
   ...
   *
   * <b>NOTE</b>: You should use a BiMap!
   *
   * @param value Value
