...
public class WseFileUtil {
  ...
  public void downloadFile(String urlStr, String out, DownloadResult result) {
    ...
    try (BufferedInputStream in = new BufferedInputStream(
        url.openStream()); FileOutputStream fos = new FileOutputStream(out)) {
  ...
  public void downloadFileWithAuthAsync(URL url, String out, String user, String pass,
      DownloadResult result) {
    downloadFileWithAuthAsync(url.toString(), out, user, pass, result);
  }

  public void downloadFileWithAuthAsync(String urlStr, String user, String pass,
      DownloadResult result) {
    executor.execute(() -> downloadFileWithAuth(urlStr, user, pass, result));
  }

  public void downloadFileWithAuthAsync(String url, String out, String user, String pass,
      DownloadResult result) {
  ...
  public void downloadFileWithAuth(String urlStr, String out, String user, String pass,
      DownloadResult result) {
    FileDownloadInfo info = new FileDownloadInfo();
    info.startedAt = (System.currentTimeMillis());
    ...
    String basicAuthPayload =
        "Basic " + Base64.getEncoder().encodeToString(userNameAndColon.getBytes());
    ...
    try {
      ...
      Files.copy(urlConnection.getInputStream(),
          Paths.get(file.getParentFile().getPath(), file.getName()),
